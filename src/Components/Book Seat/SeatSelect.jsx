...
const SeatButton = ({ id, handleChange, seatMap }) => {
  ...
  if (seatMap && seatMap[id].status) {
    if (seatMap[id].status === "selected") btnCls = "selected";
...
const SeatSelect = ({ busId, date, handleProceed }) => {
  ...
  useEffect(() => {
    (async () => {
      const res = await axios.get(
        `http://localhost:3001/bus/booked-seats?busId=${busId}&date=${date}`
      );
      const seats = res.data.seats;
      let seatMapUpdate = {};
      seats.forEach((number) => {
        seatMapUpdate[number] = { status: "booked" };
      });
      setSeatMap((prev) => ({ ...prev, ...seatMapUpdate }));
    })();
  }, []);
  ...
  return (
    <div className="select-container">
      <div className="bus-layout" key={seatMap}>
